...
export class RectangleFigure implements IBrickFigure{
    ...
    placeBricks(){
        let ff: Brick[][] = new Array();  
        for(var c: number = 0; c < this.brickColomns; c++) { 
            this.bricks[c] = [];  
            for(var r: number =0; r<this.brickRows; r++) {
                var brickX = (c*(Brick.width+this.brickPadding))+this.brickOffsetLeft;
                var brickY = (r*(Brick.height+this.brickPadding))+this.brickOffsetTop;
                this.bricks[c][r] = new Brick(brickX, brickY, "#70E852"); 
            }
        } 
    } 
}